...
export class HttpSample {
    ...
    loadFriendsSuccessFully(){
        this.result = {friends:[]};
        this.http.get('./components/http/data/friends.json').map((res: Response) => res.json()).subscribe(res => this.result = res);
    }

    loadContractByCustomer(){
        this.contract = {};
        this.customer = {};
        this.http.get('./components/http/data/customer.json').map((res: Response) => {
